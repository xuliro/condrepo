...
module.exports = function (app) {
    ...
    userController.listByCondom = function (req, res) {
        ...
        User.find({condom:_id}).populate('condom').populate('tower').populate('unit').populate("possessions").exec()
            .then(function(users){
                res.json(users);
            }, function(erro){
                console.error(erro)
    ...
    userController.listAdmsByCondom = function (req, res) {
        var _id = sanitize(req.params.id);
        User.find({condom:_id, isAdmin: true}).populate('condom').populate('tower').populate('unit').populate("possessions").exec()
            .then(function(users){
                res.json(users);
            }, function(erro){
                console.error(erro)
